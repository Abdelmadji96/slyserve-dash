...
import { ambulanceCheckExist } from "../../../../actions/ambulance";
import { AMBULANCE_CHECK_EXIST_RESET } from "../../../../actions/ambulance.types";
...
export default function RegisterAmbulance(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(ambulanceCheckExist(values.numeroTelephone, values.email));
    },
  });
  ...
  const {
    loading: loadingExistAmbulance,
    message: messageAmbulanceExist,
    error: errorExistAmbulance,
  } = useSelector((state) => state.ambulanceCheckExistReducer);
  ...
  useEffect(() => {
    if (messageAmbulanceExist === "success") {
      props.history.push("/register/position");
      dispatch({ type: AMBULANCE_CHECK_EXIST_RESET });
      dispatch({
        type: SAVE_PROFESSIONNEL,
        payload: { professionnel: formik.values, table: "ambulance" },
      });
      dispatch({
        type: SAVE_PROFESSIONNEL,
        payload: { professionnel: { ...formik.values, table: "ambulance" } },
    ...
    <div id="body" data-spy="scroll" data-target=".navbar" data-offset="100">
      ...
      <div className="content-container">
        <Container maxWidth="md">
          <Card>
            <Collapse in={loadingExistAmbulance}>
            ...
            <form onSubmit={formik.handleSubmit}>
              <CardContent>
                ...
                <Collapse in={messageAmbulanceExist === "fail"}>
              ...
              >
                <button
                  ...
                  type="submit"
                  disabled={loadingExistAmbulance || !formik.isValid}
