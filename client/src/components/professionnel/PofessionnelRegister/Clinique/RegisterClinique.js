...
import { cliniqueCheckExist } from "../../../../actions/clinique";
import { CLINIQUE_CHECK_EXIST_RESET } from "../../../../actions/clinique.types";
...
export default function RegisterClinique(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(cliniqueCheckExist(values.numeroTelephone, values.email));
    },
  });
  ...
  const {
    loading: loadingExistClinique,
    message: messageCliniqueExist,
    error: errorExistClinique,
  } = useSelector((state) => state.cliniqueCheckExistReducer);
  ...
  useEffect(() => {
    if (messageCliniqueExist === "success") {
      props.history.push("/register/position");
      dispatch({ type: CLINIQUE_CHECK_EXIST_RESET });
      dispatch({
        type: SAVE_PROFESSIONNEL,
        payload: { professionnel: { ...formik.values, table: "clinique" } },
      });
      localStorage.setItem(
        "professionnelInfos",
        JSON.stringify({ ...formik.values, table: "clinique" })
      );
    } else {
    ...
    <div id="body" data-spy="scroll" data-target=".navbar" data-offset="100">
      ...
      <div className="content-container">
        <Container maxWidth="md">
          <Card>
            <Collapse in={loadingExistClinique}>
            ...
            <form onSubmit={formik.handleSubmit}>
              <CardContent>
                ...
                <Collapse in={messageCliniqueExist === "fail"}>
              ...
              >
                <button
                  ...
                  type="submit"
                  disabled={loadingExistClinique || !formik.isValid}
