...
import { PHARMACIE_CHECK_EXIST_RESET } from "../../../../actions/pharmacie.types";
import { pharmacieCheckExist } from "../../../../actions/pharmacie";
...
export default function RegisterAmbulance(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(pharmacieCheckExist(values.numeroTelephone, values.email));
    },
  });
  ...
  const {
    loading: loadingExistPharmacie,
    message: messagePharmacieExist,
    error: errorExistAmbulance,
  } = useSelector((state) => state.pharmacieCheckExistReducer);
  ...
  useEffect(() => {
    if (messagePharmacieExist === "success") {
      props.history.push("/register/position");
      dispatch({ type: PHARMACIE_CHECK_EXIST_RESET });
      localStorage.setItem(
        "professionnelInfos",
        JSON.stringify({ ...formik.values, table: "pharmacie" })
      );
    } else {
    ...
    <div id="body" data-spy="scroll" data-target=".navbar" data-offset="100">
      ...
      <div className="content-container">
        <Container maxWidth="md">
          <Card>
            <Collapse in={loadingExistPharmacie}>
            ...
            <form onSubmit={formik.handleSubmit}>
              <CardContent>
                ...
                <Collapse in={messagePharmacieExist === "fail"}>
              ...
              >
                <button
                  ...
                  type="submit"
                  disabled={loadingExistPharmacie || !formik.isValid}
