...
//import { cliniqueSearch } from "../../actions/clinique";
import { useTranslation } from "react-i18next";
import axios from "axios";

export default function SearchBoxClinique({ setCliniques }) {
  ...
  const handleSubmit = async () => {
    ...
    const { data } = await axios.post(
      `/api/clinique/search/${communeId == "" ? "wilaya" : "wilayacommune"}`,
      {
        wilaya_id: wilayaId,
    ...
    if (data["results"]) {
      setCliniques(data["results"]);
    }
    //dispatch(cliniqueSearch({ wilayaId, communeId }));
    //}
  };
  return (
    <div>
