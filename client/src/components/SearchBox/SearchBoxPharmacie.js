...
//import { pharmacieSearch } from "../../actions/pharmacie";
import { useTranslation } from "react-i18next";
import axios from "axios";

export default function SearchBoxPharmacie({ setPharmacies }) {
  ...
  const handleSubmit = async () => {
    if (!wilayaId && !communeId) {
      ...
      const { data } = await axios.post(
        `/api/pharmacie/search/${communeId == "" ? "wilaya" : "wilayacommune"}`,
        {
          wilaya_id: wilayaId,
      ...
      setErrorSearch("");
      //dispatch(pharmacieSearch({ wilayaId, communeId }));
    }
  };
  return (
    <div>
