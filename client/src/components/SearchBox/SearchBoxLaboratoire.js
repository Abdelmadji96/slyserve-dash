...
//import { laboratoireSearch } from "../../actions/laboratoire";
import { useTranslation } from "react-i18next";
import axios from "axios";

export default function SearchBoxLaboratoire({ setLaboratoires }) {
  ...
  const handleSubmit = async () => {
    ...
    const { data } = await axios.post(
      `/api/labo/search/${communeId == "" ? "wilaya" : "wilayacommune"}`,
      {
        wilaya_id: wilayaId,
    ...
    if (data["results"]) {
      setLaboratoires(data["results"]);
    }
    //dispatch(laboratoireSearch({ wilayaId, communeId }));
    //}
  };
  return (
    <div>
